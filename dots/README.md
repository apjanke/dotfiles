...
The `install-dotfiles` script will set up the appropriate links in the current user's home directory. They will be linked to the location that `install-dotfiles` is run from. This means that you need to keep the repo around indefinitely, not just for installation. And if you relocate the repo/synced directory, existing links will point at the old location until you re-run `install-dotfiles`.

Caution: a locally-created regular file or directory is never touched -- it's reported as skipped. An existing symlink pointing somewhere else is also left alone and reported as skipped by default, UNLESS its current target is already recognizable as pointing into this repo (a rename or reorganization within the repo, say) -- pass `--relink` to fix those. A symlink pointing at an entirely different repo location (e.g. after the whole repo/synced directory relocated) isn't recognizable as ours anymore and needs `--clobber --yes-really` instead, same as any other pre-existing local content: it gets backed up to a `.bak` file, never just deleted, then replaced. `install-dotfiles` is intended to be run in a newly created account, or one that already has dotfiles managed this way.
...
- `dots/` - Configuration files meant to be installed under the user's home directory
  ...
  - `<osname>/nested/` - A mirror of `$HOME`, for tracking individual files that live inside a subdirectory you don't want to claim as a whole (e.g. one file inside `~/.config/some-app/`). Nesting can go arbitrarily deep; leaf files get symlinked, and intermediate directories are created as real directories in `~` — unless a directory is marked with `.linkdir` or `.linkdirs` (below), in which case it links whole instead. A path component starting with `_` installs with a leading `.` instead (e.g. `_config` → `.config`), so this repo never has to hold a literal dotfile that isn't actually meant to control the repo itself. A component that genuinely does start with `.` (like `.gitkeep`, or the markers themselves) is such a file, and is excluded outright: never linked, and never forces directory creation on its own. A leading `__` escapes to a literal `_`, for names that really do start with an underscore – zsh completion functions like `_git` being the main case.

A directory in `nested/` containing a `.linkdir` file links whole, the same way a `flat/` entry does, instead of being walked for individual leaf files. A directory containing a `.linkdirs` file has each of *its* immediate child directories linked whole, while the marked directory itself stays a real directory. The distinction matters where a directory needs to be one symlink (so files can come and go inside it without a re-install), but its parent has to stay real because other, unrelated content lives alongside — e.g. `~/.claude/skills/<skill>/` must each be a single symlink, while `~/.claude/` and `~/.claude/skills/` must stay real directories, since other Claude Code state and other skills live there too. A `.linkdir` at the root of a `nested/` tree is a hard error, since it would link the entire tree over `$HOME`; `.linkdirs` at the root is fine, and means "link each of these top-level directories whole."
