...
Caution: a locally-created regular file or directory is never touched -- it's reported as skipped. An existing symlink whose current target is already recognizable as pointing into this repo (a rename or reorganization within the repo, say) is force-updated by default -- pass `--no-relink` to leave those alone too, reported as skipped. A symlink pointing somewhere else entirely (e.g. after the whole repo/synced directory relocated) isn't recognizable as ours and needs `--clobber --yes-really` instead, same as any other pre-existing local content: it gets backed up to a `.bak` file, never just deleted, then replaced. `install-dotfiles` is intended to be run in a newly created account, or one that already has dotfiles managed this way.
